...
sealed class IsolateTaskResult<T> {
  ...
  IsolateTaskResult._({required this.id});
}
...
class IsolateTaskSuccessResult<T> extends IsolateTaskResult<T> {
  ...
  IsolateTaskSuccessResult({required int id, required this.data})
    : super._(id: id);
}
...
class IsolateTaskErrorResult<T> extends IsolateTaskResult<T> {
  ...
  IsolateTaskErrorResult({required int id, required this.error})
    : super._(id: id);
}
...
class IsolateTaskStreamResult<T> {
  ...
  IsolateTaskStreamResult.event({required this.id, required this.data})
    : done = false,
      error = null;

  IsolateTaskStreamResult.done({required this.id})
    : done = true,
      data = null,
      error = null;

  IsolateTaskStreamResult.error({required this.id, required this.error})
    : done = true,
      data = null;
}
...
class IsolateTaskStreamAckResult<T> extends IsolateTaskStreamResult<T> {
  IsolateTaskStreamAckResult({required super.id})
    : super._(data: null, done: false, error: null);
}
